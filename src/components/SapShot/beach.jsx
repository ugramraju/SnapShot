...
const BeachImages=()=>{
    ...
    useEffect(()=>{
        const url = `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${apikey}&tags=beaches&format=json&nojsoncallback=1`;
        axios.get(url)
    ...
    return(
        ...
        <h1>Beach Pictures</h1>
              ...
              images.map((image)=>{
                ...
                return(
                    <img key={image.id} src={imageUrl} alt="beaches"/>
                )
              })
              }
        </div>
